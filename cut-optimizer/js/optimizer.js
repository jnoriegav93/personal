...
function runOptimization() {
  ...
  let all = [];
  pieces.filter(p => !p.isSuggestion).forEach(p => { for (let q = 0; q < p.qty; q++) all.push({...p}); });
  ...
  const applyResults = () => {
    ...
    // Re-adjuntar piezas sugeridas al canvas (tienen info de posición guardada)
    pieces.filter(p => p.isSuggestion && p._sheetIdx !== undefined).forEach(p => {
      const sh = optimized[p._sheetIdx];
      if (!sh) return;
      sh.suggestions.push({ name: p.name, rw: p._rw, rh: p._rh, x: p._x, y: p._y, rotated: p._rotated, cuts: p._cuts || {cols:1,rows:1} });
    });
