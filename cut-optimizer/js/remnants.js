...
function renderRemnants() {
  ...
  allRemnants.forEach(r => {
    html += `<div class="remnant-card">
      ...
      <button class="rem-add-btn" onclick="openAddSuggestionModal(${r.w},${r.h},${r.sheet-1})">+ Agregar a plancha</button>
    </div>`;
  });
  html += '</div>';
...
let remnantPanelHeight = 280;
function toggleRemnant() {
  const panel   = document.getElementById('remnantPanel');
  ...
  if (remnantOpen) {
    ...
    panel.style.height   = remnantPanelHeight + 'px';
    panel.style.overflow = '';
    chevron.classList.add('open');
    remnantOpen = true;
    // Siempre re-renderizar al abrir para asegurar contenido fresco
